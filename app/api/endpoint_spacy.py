...
from app.utils.preprocessing_spacy import preprocess_data, preprocess_text, train_model, text_to_bow
...
@router_spacy.post("/similarity", response_model=SimilarityResponse)
...
class ModelTrainer:
    ...
    def load_model(self):
        model_path = os.path.join(self.model_folder, "naive_bayes_classifier_spacy.pickle")
...
@router_spacy.get("/train_model/spacy/")
def retrain_model():
    logging.info("Start")
    ...
    end_time = time.time()
    logging.info(f"data is preprocessed, time: {end_time - start_time}, time: 635.6752200126648")
    ...
    if model_folder.exists() and model_folder.is_dir():
        model_filename = model_folder / "naive_bayes_classifier_spacy.pickle"
...
@router_spacy.post("/predict/spacy/")
